...
设置页的 API Keys 区块现在只保留 OpenAI 相关配置：`OpenAI API Key`、`OpenAI Base URL`、`OpenAI-Compatible Model ID`。`Anthropic API Key` 和 `Google/Gemini API Key` 已经从 `app/src/settings_view/ai_page.rs` 移除；后续如果再做 BYOK UI，默认沿着“统一走 OpenAI-compatible 入口”思路扩展，而不是把多家 provider 的独立 key 输入框再加回来。
