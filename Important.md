...
`app/src/ai/agent/api/local_openai/request.rs` 在把 `AIAgentContext::Block` 序列化进用户上下文时，会把 `block.output` 截断到最多 20000 个字符。这里的目标是防止单个 shell block 输出把 prompt 撑得过大；如果后续觉得上下文“莫名少了一截”，先确认是不是命中了这条上限。
