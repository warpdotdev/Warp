...
2. On Linux Wayland, the same settings area provides a "System shortcut command" path for users who want global-key behavior through their compositor:
   ...
   - It states that custom shortcuts on GNOME and KDE normally do not provide Warp with a Wayland activation token; launch and hide/minimize behavior still work where supported, but showing/focusing an already-running window may only request attention or leave the current focus unchanged.
