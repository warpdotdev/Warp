...
- Behavior #7, #11, #17: perform manual Wayland validation on GNOME 45+ and KDE Plasma 5.27+ custom shortcuts. Record compositor/window-manager versions, whether `XDG_ACTIVATION_TOKEN` or equivalent activation metadata is present, and whether activation is granted, denied, attention-requested, or minimized for each desktop. On GNOME 48+/49+, KDE 5.27+, or Hyprland setups where the `GlobalShortcuts` portal exists, verify this command does not implicitly depend on portal registration unless that separate follow-up has been implemented.
