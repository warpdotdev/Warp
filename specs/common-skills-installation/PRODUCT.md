...
21. If the Warp checkout has no common-skills lock during a normal install flow, the installer creates one from `warpdotdev/common-skills` and installs the corresponding common skills into the selected target. This supports first-time setup of a checkout that has not yet adopted a common-skills lock.
