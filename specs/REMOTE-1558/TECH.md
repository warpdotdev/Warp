...
Introduce a small client-side launch DTO shared by `Input`, slash commands, workspace, and ambient model code. Put it in the handoff module at `app/src/ai/blocklist/handoff/launch.rs`, and re-export it from `app/src/ai/blocklist/handoff/mod.rs`.
...
- `CloudLaunchSubmitMode` — `AutoSubmit` for `& query` and `/handoff query`. (No-query `/handoff` and the chip activate `&` compose mode instead of dispatching a request.)
...
- `WorkspaceAction::OpenLocalToCloudHandoffPane { request: CloudLaunchRequest }`
The footer chip activates `&` handoff-compose mode on the local input by calling `Input::activate_cloud_handoff_compose` rather than dispatching a `CloudLaunchRequest`. `/handoff` with no query also activates `&` handoff-compose mode, same as the chip. `/handoff query` dispatches `AutoSubmit` with the parsed prompt, collected image/file attachments, and no explicit environment. `& query` dispatches `AutoSubmit` with the stripped prompt, collected image/file attachments, and the explicit environment id from `HandoffComposeState`, if the user chose one.
...
`execute_slash_command` currently clears every handled slash command after dispatch. `/handoff query` needs a deferred-clear path when it dispatches a `CloudLaunchRequest`; otherwise a pre-pane failure would lose `/handoff query`.
Add a boolean flag (e.g. `defer_buffer_clear_until_cloud_launch_claim`) in the slash-command execution path so `/handoff query` can say "handled, but source clear is owned by workspace claim." `/handoff` with no query activates `&` compose mode and lets the normal slash-command buffer clear proceed (the buffer is cleared to make room for the compose draft). When workspace claims a `/handoff query` launch, it clears the source buffer through the same `CloudLaunchRequestId` mechanism used by `&`.
### 8. End-to-end flow
```mermaid
sequenceDiagram
...
- **Input clears before workspace owns the launch.** The request-id claim protocol keeps source prompt and attachments intact through validation and fork failures. Tests should cover both `&` and `/handoff query`.
...
- `app/src/terminal/input/slash_commands/*`: test that `/handoff query` uses the deferred-clear launch effect and `/handoff` without query activates `&` compose mode.
...
## Follow-ups
- Add general cloud-mode serialization for selected text/block/document context and plumb it through `SpawnAgentRequest.referenced_attachments` or the appropriate server contract. Once that exists, `CloudLaunchRequest` should carry the same selected-context payload for normal cloud mode, `&`, and `/handoff query`.
