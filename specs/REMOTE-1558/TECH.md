...
Add a boolean flag (e.g. `defer_buffer_clear_until_cloud_launch_claim`) in the slash-command execution path so `/move-to-cloud` can say “handled, but source clear is owned by workspace claim.” Other slash commands should keep the existing clear behavior. When workspace claims a slash-command launch, it clears the source buffer through the same `CloudLaunchRequestId` mechanism used by `&`.
### 8. End-to-end flow
```mermaid
sequenceDiagram
