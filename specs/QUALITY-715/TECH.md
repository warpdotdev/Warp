...
4. Set the suppress policy in parent-owned child pane creation paths:
   ...
   - Set the policy for restored remote child panes in `PaneGroup::create_hidden_child_agent_pane` when `child_conversation.is_remote_child()` is true. These panes are parent-owned Cloud Agent views for a remote child; their details panel can auto-open later on `AmbientAgentViewModelEvent::SessionReady`/`FollowupSessionReady`, after environment initialization finishes.
   - Set the policy directly on the hidden ambient-agent child pane creation helper `PaneGroup::insert_ambient_agent_pane_hidden_for_child_agent` so that fresh remote child panes created by the local-orchestrator `StartAgentExecutionMode::Remote` path (`launch_remote_child` in `app/src/pane_group/pane/terminal_pane.rs`) are also suppressed before the environment setup loading screen and subsequent `AmbientAgentViewModelEvent::SessionReady`/`FollowupSessionReady` events run. This covers both restored remote children (via `create_hidden_child_agent_pane`) and freshly-spawned remote children (via `launch_remote_child`).
