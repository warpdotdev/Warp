...
## Goals
Restore remote codebase index metadata for the remote-server daemon without reading or restoring unrelated app session/cloud/UI persistence.
...
Do not make the daemon restore full app state such as panes, cloud objects, command history, user profiles, MCP servers, or projects.
...
`persistence::initialize` accepts a `PersistenceScope` so startup can choose between the normal app scope and the remote-server daemon scope. `PersistenceScope::App` restores the full `PersistedData` payload that the GUI app needs. `PersistenceScope::RemoteServerDaemon { identity_key }` uses a daemon-specific database path derived from the remote-server identity and restores only codebase index metadata.
`RestoredPersistenceData` keeps those restore products explicit:
- `App(Box<PersistedData>)` for normal app startup.
- `RemoteCodebaseIndexing(Vec<CodeWorkspaceMetadata>)` for daemon indexing startup.
This split is intentional. The daemon needs the same Diesel/SQLite open, migrate, writer, and codebase-index metadata machinery as the app, but it should not deserialize or retain unrelated app-scoped state. Reading the app-scoped database wholesale would couple the headless daemon to UI/session/cloud-object restore code, require app-only singleton assumptions, and make remote daemon storage harder to reason about. The narrow restore payload keeps the daemon boundary explicit while still reusing the existing persistence infrastructure.
...
The daemon only needs enough persisted data to reconstruct codebase-indexing state:
...
That narrow restore is still “syncing app data” in the sense that it reuses the same `codebase_indices` model that local app startup uses, but it is not syncing the entire app database object graph. This is the intended boundary: share the data model and persistence infrastructure for codebase indexing, not the whole app restore contract.
...
`StartupPersistenceData::from_restored_persistence_data` normalizes both restore payloads into the startup fields consumed later by singleton registration. For daemon restore, it only fills `persisted_workspaces` and leaves the full app fields empty. That lets the existing `CodebaseIndexManager` constructor receive daemon-restored index metadata without pretending the daemon has a full app session restore.
...
- Local can restore full app state because it is the app; the daemon restores only codebase-index data because it is a headless worker.
...
Persistence coverage should verify that daemon scope restores only codebase index metadata and that app scope still restores the full `PersistedData` payload. Startup-level coverage should verify that daemon launch passes daemon-scoped snapshot storage into `CodebaseIndexManager::new_with_snapshot_storage`.
...
### Daemon accidentally restores app state
Restoring full app data in the daemon would couple daemon startup to UI/session state and app-only singleton assumptions. Mitigate by keeping `RestoredPersistenceData::RemoteCodebaseIndexing` narrow and making daemon startup consume only `persisted_workspaces`.
...
## Definition of done
Remote daemon startup can restore known codebase index metadata without restoring unrelated app state.
