...
- `SNAPSHOT_UPLOAD_TECH.md` — handoff snapshot prep + upload pipeline and the `SpawnAgentRequest` server-contract additions.
## Context
...
sequenceDiagram
    ...
    API->>DB: GetActiveExecutionForRun → Input.HandoffPrepToken=<token>
...
See `TOUCHED_WORKSPACE_TECH.md` for the path-extraction walk, repo grouping, and env-overlap pick. The orchestrator described in §2 calls `extract_paths_from_conversation` and `derive_touched_workspace` on chip click, and applies `pick_handoff_overlap_env` once derivation completes.
...
- `fetch_and_download_handoff_snapshot_attachments` (`app/src/ai/agent_sdk/driver/attachments.rs:68`) calls `GET /agent/runs/:runId/handoff/attachments`. The server reads the active execution's `Input.HandoffPrepToken` and lists files at `handoff_prep/{token}/`; if the token is absent (post-first-execution retries, cloud→cloud handoffs) it falls back to the latest ENDED execution's `snapshots/{run_id}/{exec_id}/` upload.
...
- Server flag (`handoff_local_cloud_enabled`) goes Dogfood first, end-to-end tested with a Warp engineer's local→cloud handoff against a staging worker.
