...
6. Add Nushell-safe update command construction.
   ...
   - Tests must include malicious-looking dynamic values containing spaces, quotes, semicolons, `$()`, newlines, and leading dashes, and assert those values remain a single Nushell string/argument without introducing extra statements or moving `warp_finish_update` outside the success gate.
...
- Behavior 13: `app/src/autoupdate/linux_test.rs` verifies the Nushell update command gates `warp_finish_update` behind the package-manager command sequence, emits the expected outer `try { ... } catch { ... }` shape, preserves best-effort dist-upgrade handling for Apt, does not emit POSIX `&&`, and safely quotes/binds dynamic package, repository, update-id, cache, and path values before inserting them into privileged command blocks, including control characters that JSON would encode as invalid Nushell `\u00xx` escapes.
