...
workspace-view-all-cloud-runs = すべてのエージェント実行を表示
...
settings-teams-create-description = チームを作成すると、ローカルのエージェント実行・環境・自動化・成果物を共有してエージェント主導の開発で協業できます。チームメイトとエージェント双方のための共有ナレッジストアも作成できます。
