...
pub mod entry {
    ...
    use super::{
        ...
        CreatedOnFilter, CreatorFilter, EnvironmentFilter, HarnessFilter, OwnerFilter,
...
pub use entry::{
    AgentConversationBacking, AgentConversationCapabilities, AgentConversationCreator,
...
impl AgentConversationsModel {
    ...
    pub fn get_entries(
        ...
        for task in self.tasks.values() {
            let entry = Self::entry_for_task(task, history_model, app);
        ...
        for metadata in self.conversations.values() {
            ...
            let entry = Self::entry_for_conversation(metadata, history_model, app);
        ...
        for metadata in history_model.get_local_conversations_metadata() {
            ...
            entries.push(Self::entry_for_historical_metadata(
                metadata,
                nav_data,
                history_model,
                app,
    ...
    fn entry_for_task(
        ...
        let local_conversation_id = Self::conversation_id_shadowed_by_task(task, history_model);
        ...
        let server_conversation_token = task
            ...
            .or_else(|| {
                local_conversation_id.and_then(|conversation_id| {
                    Self::server_conversation_token_for_conversation(
                        conversation_id,
                        None,
                        history_model,
                    )
        ...
        AgentConversationEntry {
            ...
            backing: AgentConversationBacking {
                ...
                has_cloud_data: conversation_metadata
                    .is_some_and(|metadata| metadata.has_cloud_data)
    ...
    fn entry_for_historical_metadata(
        metadata: &AIConversationMetadata,
        nav_data: ConversationNavigationData,
        history_model: &BlocklistAIHistoryModel,
        app: &AppContext,
    ) -> AgentConversationEntry {
        Self::entry_for_conversation_parts(nav_data, Some(metadata), history_model, app)
    }
    ...
    fn entry_for_conversation_parts(
        ...
        AgentConversationEntry {
            ...
            identity: AgentConversationIdentity {
                ...
                server_conversation_token: Self::server_conversation_token_for_conversation(
                    conversation_id,
                    Some(&metadata.nav_data),
                    history_model,
                ),
            ...
            display: AgentConversationDisplayData {
                ...
                harness: conversation_metadata
                    .and_then(|metadata| metadata.server_conversation_metadata.as_ref())
                    .map(|metadata| Self::harness_from_server_harness(metadata.harness))
    ...
    fn harness_from_server_harness(harness: AIAgentHarness) -> Harness {
        match harness {
