...
// ── note_missing_tab_config_profile tests ───────────────────────────
//
// These tests cover the per-app-session dedup the model maintains for
// missing-tab-config-profile toasts. Both the immediate launch path
// (`pane_group::PaneGroup::apply_tab_config_agent_profile`) and the
// deferred launch path (`TerminalView::apply_pending_agent_profile`)
// call this method to coordinate so a tab config that references the
// same missing profile from multiple panes only produces one toast,
// even when those panes mix immediate and deferred entry.

#[test]
fn note_missing_tab_config_profile_first_time_returns_true() {
    App::test((), |mut app| async move {
        install_singletons(&mut app, AuthStateProvider::new_for_test());
        let profile_model = app.add_singleton_model(|ctx| {
            AIExecutionProfilesModel::new(&LaunchMode::new_for_unit_test(), ctx)
        });
        profile_model.update(&mut app, |model, _| {
            assert!(
                model.note_missing_tab_config_profile("Coder".to_string()),
                "first call for a name should return true so the caller emits the toast",
            );
        });
    })
}
...
#[test]
fn note_missing_tab_config_profile_repeat_returns_false() {
    App::test((), |mut app| async move {
        install_singletons(&mut app, AuthStateProvider::new_for_test());
        let profile_model = app.add_singleton_model(|ctx| {
            AIExecutionProfilesModel::new(&LaunchMode::new_for_unit_test(), ctx)
        ...
        profile_model.update(&mut app, |model, _| {
            ...
            assert!(
                !model.note_missing_tab_config_profile("Coder".to_string()),
                "second call for the same name should return false so the caller suppresses the toast",
            );
        });
    })
}
...
#[test]
fn note_missing_tab_config_profile_distinct_names_independent() {
    App::test((), |mut app| async move {
        install_singletons(&mut app, AuthStateProvider::new_for_test());
        let profile_model = app.add_singleton_model(|ctx| {
            AIExecutionProfilesModel::new(&LaunchMode::new_for_unit_test(), ctx)
        ...
        profile_model.update(&mut app, |model, _| {
            assert!(model.note_missing_tab_config_profile("Coder".to_string()));
            assert!(model.note_missing_tab_config_profile("Reviewer".to_string()));
            assert!(
                !model.note_missing_tab_config_profile("Coder".to_string()),
                "Coder was already warned; second call should return false",
            );
            assert!(
                !model.note_missing_tab_config_profile("Reviewer".to_string()),
                "Reviewer was already warned; second call should return false",
            );
        });
    })
}
