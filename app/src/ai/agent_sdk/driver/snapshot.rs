...
use crate::server::server_api::ai::{
    AIClient, HandoffPrepToken, SnapshotUploadFileInfo as AiSnapshotUploadFileInfo,
...
/// contents, allocate a `handoff_prep_token` plus presigned upload URLs via
/// `AIClient::upload_local_handoff_snapshot`, and upload the artifacts.
///
/// Returns:
/// - `Ok(Some(handoff_prep_token))` when a token was minted, regardless of how many blobs
///   landed. The cloud-side `RunAgentRequest` should be sent with this token even if individual
...
pub(crate) async fn upload_snapshot_for_handoff(
    ...
    let response = client
        ...
        .await
        .context("failed to allocate handoff_prep_token")?;
    log::info!(
        "handoff_prep_token allocated; expires_at={}, uploads={}",
        response.expires_at,
        response.uploads.len(),
    );
    let handoff_prep_token = response.handoff_prep_token;
    ...
    .await
    {
        ...
        if summary.uploaded == 0 && !summary.manifest_uploaded && summary.total > 0 {
            report_error!(anyhow::anyhow!(
                "All handoff snapshot uploads failed; cloud agent will start with no rehydration content"
            ));
        }
    }

    Ok(Some(handoff_prep_token))
}
