...
pub struct SpawnAgentRequest {
    ...
    /// References a batch of files previously uploaded to handoff_prep/{handoff_prep_token}/
    /// via `POST /agent/handoff/upload-snapshot`. The server stores the token on the new run's
    /// queued execution input and resolves the prefix in place at rehydration time — no
    /// GCS move happens.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub handoff_prep_token: Option<HandoffPrepToken>,
}

/// Server-minted token returned by `POST /agent/handoff/upload-snapshot` that scopes a batch
/// of presigned upload URLs to `handoff_prep/{handoff_prep_token}/`. The client passes it
/// back via `SpawnAgentRequest.handoff_prep_token`; the server stores it on the new run's
/// queued execution input so rehydration discovery can read the same prefix.
#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
pub struct HandoffPrepToken(String);

impl HandoffPrepToken {
...
/// handoff flow (REMOTE-1486) to allocate a `handoff_prep_token` and presigned upload URLs
/// scoped to `handoff_prep/{handoff_prep_token}/` before any task exists.
#[derive(Debug, Clone, serde::Serialize)]
...
pub struct UploadLocalHandoffSnapshotResponse {
    pub handoff_prep_token: HandoffPrepToken,
...
pub trait AIClient: 'static + Send + Sync {
    ...
    /// Allocate a `handoff_prep_token` and presigned upload URLs for staging local-to-cloud
