...
impl AuthViewBody {
    ...
    fn render_privacy_information(
        ...
        let disclaimer_line_2 = if FeatureFlag::GlobalAIAnalyticsBanner.is_enabled() {
            ...
            Flex::column()
                .with_child(
                    ui_builder
                        .paragraph(t!("auth.opt_out_analytics_ai").to_string())
                        .with_style(disclaimer_styles)
                        .build()
                        .finish(),
                )
                .with_child(
                    Flex::row()
                        .with_child(
                            ui_builder
                                .paragraph(t!("auth.adjust_your_prefix").to_string())
                                .with_style(disclaimer_styles)
                                .build()
                                .finish(),
                        )
    ...
    fn render_sign_in_row(&self, ui_builder: &UiBuilder) -> Box<dyn Element> {
        Flex::row()
            .with_child(
                ui_builder
                    .span(t!("auth.already_have_account").to_string())
                    .build()
                    .finish(),
            )
            .with_child(
    ...
    fn render_sign_up_later_row(&self, ui_builder: &UiBuilder) -> Box<dyn Element> {
        Container::new(
            Flex::row()
                .with_child(
                    ui_builder
                        .span(t!("auth.dont_want_sign_in_now").to_string())
                        .build()
                        .finish(),
                )
                .with_child(
    ...
    fn render_sign_in_later_confirm_row(&self, ui_builder: &UiBuilder) -> Box<dyn Element> {
        Container::new(
            Flex::column()
                .with_child(
                    ui_builder
                        .paragraph(t!("auth.skip_login_confirm_title").to_string())
                        .build()
                        .finish(),
                )
                .with_child(
                    ui_builder
                        .paragraph(t!("auth.skip_login_confirm_body_1").to_string())
                        .build()
                        .finish(),
                )
                .with_child(
                    Flex::row()
                        .with_child(
                            ui_builder
                                .span(t!("auth.skip_login_confirm_body_2").to_string())
                                .build()
                                .finish(),
                        )
                        .with_child(
                            ui_builder
                                .link(
                                    t!("auth.yes_skip_login").to_string(),
                                    None,
                                    Some(Box::new(|ctx| {
    ...
    fn render_force_login_disclaimer(
        ...
        let text = match self.variant {
            AuthViewVariant::RequireLoginCloseable => {
                t!("auth.require_login_ai_collab").to_string()
            }
            AuthViewVariant::HitDriveObjectLimitCloseable => {
                t!("auth.require_login_drive_limit").to_string()
            }
            AuthViewVariant::ShareRequirementCloseable => {
                t!("auth.require_login_share").to_string()
            }
            _ => String::new(),
        };
    ...
    fn render_header(&self, appearance: &Appearance, ui_builder: &UiBuilder) -> Box<dyn Element> {
        ...
        let text = match self.variant {
            AuthViewVariant::Initial => t!("auth.welcome_to_warp").to_string(),
            AuthViewVariant::RequireLoginCloseable
            | AuthViewVariant::HitDriveObjectLimitCloseable
            | AuthViewVariant::ShareRequirementCloseable => t!("auth.sign_up_for_warp").to_string(),
        };
    ...
    fn render_browser_open_content(
        ...
        let header = Container::new(
            ui_builder
                .paragraph(t!("auth.sign_in_browser_to_continue").to_string())
                .with_style(header_styles)
                .build()
                .finish(),
        )
...
impl View for AuthViewBody {
    ...
    fn accessibility_contents(&self, _: &AppContext) -> Option<AccessibilityContent> {
        Some(AccessibilityContent::new(
            t!("auth.welcome_to_warp").to_string(),
            t!("auth.accessibility_open_browser").to_string(),
