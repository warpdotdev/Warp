...
/// Returns total `(lines_added, lines_removed)` for everything this branch
/// has changed vs the detected main branch — both committed and
/// uncommitted — plus a count of lines in any untracked text files.
///
/// This is the right summary for branch / worktree workflows where an
/// agent commits its work as it goes: `git diff --shortstat HEAD` would
/// report `0` immediately after each commit, masking the real magnitude
/// of the agent's changes. Diffing against the main branch instead
/// gives the cumulative "what would land in a PR" change count.
///
/// When already on the main branch (or main can't be detected) this
/// falls back to the HEAD-relative semantics of
/// [`get_repo_git_summary`] so plain repo views still show uncommitted
/// edits.
///
/// Returns `None` when the diff and untracked counts are both zero, or
/// when the path is not a git repo / git is unavailable.
#[cfg(feature = "local_fs")]
pub async fn get_branch_change_summary(repo_root: &Path) -> Option<(u32, u32)> {
    use crate::context_chips::display_chip::GitLineChanges;

    // Pick the diff base: the main branch (so committed-since-fork and
    // uncommitted both count) when we're on a non-main branch,
    // otherwise HEAD (so we still report uncommitted edits when the user
    // is on the main branch directly).
    let main_branch = detect_main_branch(repo_root)
        .await
        .ok()
        .map(|s| s.trim().to_string())
        .filter(|s| !s.is_empty());
    let current_branch = detect_current_branch(repo_root)
        .await
        .ok()
        .map(|s| s.trim().to_string())
        .filter(|s| !s.is_empty() && s != "HEAD");
    let on_main = match (current_branch.as_deref(), main_branch.as_deref()) {
        (Some(current), Some(main)) => {
            current == main || current == main.trim_start_matches("origin/")
        }
        _ => false,
    };
    let base: String = match (&main_branch, on_main) {
        (Some(main), false) => main.clone(),
        _ => "HEAD".to_string(),
    };

    log::debug!("[GIT OPERATION] git.rs get_branch_change_summary git diff --shortstat {base}");
    let stats = run_git_command(repo_root, &["diff", "--shortstat", &base])
        .await
        .ok()
    ...
    // Add lines from untracked files so unstaged work-in-progress in
    // brand-new files still shows up in the summary (parity with
    // `get_repo_git_summary`).
    log::debug!(
        "[GIT OPERATION] git.rs get_branch_change_summary git ls-files --others --exclude-standard"
    );
...
#[cfg(not(feature = "local_fs"))]
pub async fn get_branch_change_summary(_repo_root: &Path) -> Option<(u32, u32)> {
    None
}
