...
impl CodePageWidget {
        ...
        if workspaces.is_empty() {
            content.add_child(
                Container::new(
                    appearance
                        .ui_builder()
                        .paragraph(i18n::tr(app, I18nKey::CodeNoFoldersInitialized))
                        .build()
                        .finish(),
                )
    ...
    fn render_indexing_subsection(
        ...
        column.add_child(
            ui_builder
                .span(i18n::tr(app, I18nKey::CodeIndexingSection))
                .with_style(UiComponentStyles {
                    font_size: Some(11.0),
                    font_weight: Some(Weight::Semibold),
                    font_color: Some(theme.disabled_ui_text_color().into()),
        ...
        if let Some(index_status) = index_status {
            ...
            column.add_child(
                Flex::row()
                    ...
                    .with_child(
                        ui_builder
                            .label(i18n::tr(app, I18nKey::CodeNoIndexCreated))
                            .with_style(UiComponentStyles {
        ...
        let (status_text, status_color) = if index_state.has_pending() {
            let progress_text = match index_state.sync_progress() {
                Some(SyncProgress::Discovering { total_nodes }) => Cow::from(
                    i18n::tr(app, I18nKey::CodeIndexDiscoveredChunks)
                        .replace("{count}", &total_nodes.to_string()),
                ),
                Some(SyncProgress::Syncing {
                    completed_nodes,
                    total_nodes,
                }) => Cow::from(
                    i18n::tr(app, I18nKey::CodeIndexSyncingProgress)
                        .replace("{completed}", &completed_nodes.to_string())
                        .replace("{total}", &total_nodes.to_string()),
                ),
                None => Cow::from(i18n::tr(app, I18nKey::CodeIndexSyncing)),
            };
            ...
            let (text, color, status_icon) = if completed_successfully {
                (
                    i18n::tr(app, I18nKey::CodeIndexSynced),
                    theme.ansi_fg_green(),
                    Icon::Check,
                )
            ...
            {
                (
                    i18n::tr(app, I18nKey::CodeIndexTooLarge),
                    theme.ui_warning_color(),
                    Icon::AlertTriangle,
                )
            } else if index_state.has_synced_version() {
                (
                    i18n::tr(app, I18nKey::CodeIndexStale),
            ...
            (
                Cow::from(i18n::tr(app, I18nKey::CodeIndexNotBuilt)),
