...
impl DeleteEnvironmentConfirmationDialog {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        let cancel_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(i18n::t("button.cancel"), NakedTheme).on_click(|ctx| {
                ctx.dispatch_typed_action(DeleteEnvironmentConfirmationDialogAction::Cancel);
            })
        });

        let confirm_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(i18n::t("settings.delete_env.confirm"), DangerPrimaryTheme).on_click(|ctx| {
...
impl View for DeleteEnvironmentConfirmationDialog {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let description = i18n::t("settings.delete_env.description").replace("{env_name}", &self.env_name);

        let dialog = Dialog::new(
            i18n::t("settings.delete_env.title").to_string(),
            Some(description),
