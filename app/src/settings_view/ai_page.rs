...
fn next_command_description() -> &'static str { i18n::t("settings.ai.next_command.description") }
fn prompt_suggestions_description() -> &'static str { i18n::t("settings.ai.prompt_suggestions.description") }
fn suggested_code_banners_description() -> &'static str { i18n::t("settings.ai.suggested_code_banners.description") }
fn natural_language_autosuggestions() -> &'static str { i18n::t("settings.ai.natural_language_autosuggestions.description") }
fn shared_block_title_generation_description() -> &'static str { i18n::t("settings.ai.shared_block_title_generation.description") }
fn git_operations_autogen_description() -> &'static str { i18n::t("settings.ai.git_operations_autogen.description") }
...
impl AISettingsPageView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let code_read_allowlist_editor = ctx.add_typed_action_view(|ctx| {
            ...
            input.set_placeholder_text(i18n::t("settings.ai.placeholder.code_repo"), ctx);
            input
        });
        Self::update_editor_interaction_state(
        ...
        let autodetection_denylist_editor = ctx.add_typed_action_view(|ctx| {
            ...
            editor.set_placeholder_text(i18n::t("settings.ai.placeholder.commands_comma"), ctx);
        ...
        let command_execution_allowlist_editor = ctx.add_typed_action_view(|ctx| {
            ...
            input.set_placeholder_text(i18n::t("settings.ai.placeholder.regex_ls"), ctx);
            input
        });
        Self::update_editor_interaction_state(
        ...
        let command_execution_denylist_editor = ctx.add_typed_action_view(|ctx| {
            let mut input =
                SubmittableTextInput::new(ctx).validate_on_edit(|s| Regex::new(s).is_ok());
            input.set_placeholder_text(i18n::t("settings.ai.placeholder.regex_rm"), ctx);
            input
        });
        Self::update_editor_interaction_state(
        ...
        let cli_agent_footer_command_editor = ctx.add_typed_action_view(|ctx| {
            let mut input =
                SubmittableTextInput::new(ctx).validate_on_edit(|s| Regex::new(s).is_ok());
            input.set_placeholder_text(i18n::t("settings.ai.placeholder.command_regex"), ctx);
            input
        });
        ...
        let write_to_pty_autonomy_dropdown_menu = ctx.add_typed_action_view(|ctx| {
            ...
            dropdown.set_items(
                vec![
                    ...
                    DropdownItem::new(
                        i18n::t("settings.ai.option.ask_on_first_write"),
        ...
        let mcp_allowlist_dropdown = ctx.add_typed_action_view(|ctx| {
            ...
            dropdown.set_top_bar_max_width(AI_SETTINGS_DROPDOWN_WIDTH);
            dropdown.set_menu_width(AI_SETTINGS_DROPDOWN_WIDTH, ctx);
            dropdown.set_menu_header_to_static(i18n::t("settings.ai.select_mcp_servers"));
            dropdown
        });
        ...
        let conversation_layout_dropdown = ctx.add_typed_action_view(|ctx| {
            ...
            match current {
                OpenConversationPreference::NewTab => dropdown.set_selected_by_name(i18n::t("settings.ai.option.new_tab"), ctx),
                OpenConversationPreference::SplitPane => {
                    dropdown.set_selected_by_name(i18n::t("settings.ai.option.split_pane"), ctx)
                }
            };
            dropdown
        });
    ...
    fn refresh_autonomy_dropdown_menu(
        ...
        menu.update(ctx, |menu, ctx| {
            ...
            menu.set_items(
                vec![
                    DropdownItem::new(
                        i18n::t("settings.ai.option.read_only"),
                        AISettingsPageAction::SetAutonomyReadonlyCommandsSetting,
                    ),
                    DropdownItem::new(
                        i18n::t("settings.ai.option.supervised"),
        ...
        menu.update(ctx, |menu, ctx| {
            ...
            menu.set_items(
                AgentModeCodingPermissionsType::iter()
                    .map(|t| {
                        let display = match t {
                            AgentModeCodingPermissionsType::AlwaysAskBeforeReading => i18n::t("settings.ai.option.always_ask"),
                            AgentModeCodingPermissionsType::AlwaysAllowReading => i18n::t("settings.ai.option.always_allow"),
                            AgentModeCodingPermissionsType::AllowReadingSpecificFiles => {
                                i18n::t("settings.ai.option.allow_in_specific_directories")
                            }
                        };
            ...
            .map(|(pattern_clone, current_agent)| {
                ctx.add_typed_action_view(move |ctx| {
                    ...
                    dropdown.set_menu_header_text_override(|label| {
                        if label == "Other" {
                            i18n::t("settings.ai.select_coding_agent").to_string()
                        } else {
                            label.to_string()
                        }
                    });
...
fn render_toolbar_layout_editor(
    ...
    let label = Container::new(
        appearance
            .ui_builder()
            .span(i18n::t("settings.ai.toolbar_layout").to_string())
            .with_style(UiComponentStyles {
    ...
    fn render(
        ...
        if is_ai_disabled_due_to_remote_session_org_policy {
            row.add_child(
                ConstrainedBox::new(
                    Container::new(
                        Text::new(i18n::t("settings.ai.org_disallows_remote_ai"), appearance.ui_font_family(), 12.)
                    ...
                    .with_child(
                        Container::new(
                            ui_builder
                                ...
                                .with_text_label(i18n::t("button.sign_up").to_owned())
                                .build()
                                .on_click(move |ctx, _, _| {
...
impl UsageWidget {
    fn render_request_usage_count(
        ...
        let request_count_label = if workspace_is_delinquent_due_to_payment_issue {
            i18n::t("settings.ai.restricted_billing").to_string()
        } else if is_unlimited {
            i18n::t("settings.ai.unlimited").to_string()
        } else {
            format!("{used}/{limit}")
        };
...
impl SettingsWidget for UsageWidget {
    ...
    fn render(
        ...
        let usage_header = Container::new(
            Flex::row()
                ...
                .with_child(
                    appearance
                        .ui_builder()
                        .paragraph(i18n::t("settings.ai.resets").replace("{formatted_next_refresh_time}", &formatted_next_refresh_time))
                        .with_style(UiComponentStyles {
        ...
        let request_limit_description = i18n::t("settings.ai.credits_limit_description").replace("{0}", &ai_request_usage_model.refresh_duration_to_string());

        let request_usage_row = self.render_ai_usage_limit_row(
            i18n::t("settings.ai.credits"),
            ...
            UserWorkspaces::as_ref(app).current_team()
        {
            ...
            if team.billing_metadata.can_upgrade_to_higher_tier_plan() {
                ...
                if has_admin_permissions {
                    vec![
                        FormattedTextFragment::hyperlink(i18n::t("settings.ai.upgrade"), upgrade_url),
                        FormattedTextFragment::plain_text(i18n::t("settings.ai.get_more_usage")),
                    ]
                } else {
                    // The /upgrade page says to contact their administrator.
                    vec![
                        FormattedTextFragment::hyperlink(i18n::t("settings.ai.compare_plans"), upgrade_url),
                        FormattedTextFragment::plain_text(i18n::t("settings.ai.more_usage")),
                    ]
                }
            } else {
                vec![
                    FormattedTextFragment::hyperlink(i18n::t("settings.ai.contact_support"), "mailto:support@warp.dev"),
                    FormattedTextFragment::plain_text(i18n::t("settings.ai.more_usage")),
                ]
            }
        } else {
...
impl ActiveAIWidget {
        ...
        Flex::column()
            .with_child(
                render_ai_setting_toggle::<IntelligentAutosuggestionsEnabled>(
                    i18n::t("settings.ai.next_command"),
                    AISettingsPageAction::ToggleIntelligentAutosuggestions,
        ...
        Flex::column()
            .with_child(
                render_ai_setting_toggle::<AgentModeQuerySuggestionsEnabled>(
                    i18n::t("settings.ai.prompt_suggestions"),
                    AISettingsPageAction::TogglePromptSuggestions,
    ...
    fn render_suggested_code_banners_section(
        ...
        Flex::column()
            .with_child(
                render_ai_setting_toggle::<AgentModeQuerySuggestionsEnabled>(
                    i18n::t("settings.ai.suggested_code_banners"),
                    AISettingsPageAction::ToggleCodeSuggestions,
    ...
    fn render_natural_language_autosuggestions_section(
        ...
        Flex::column()
            ...
            >(
                i18n::t("settings.ai.natural_language_autosuggestions"),
                AISettingsPageAction::ToggleNaturalLanguageAutosuggestions,
        ...
        Flex::column()
            .with_child(
                render_ai_setting_toggle::<SharedBlockTitleGenerationEnabled>(
                    i18n::t("settings.ai.shared_block_title_generation"),
                    AISettingsPageAction::ToggleSharedTitleGeneration,
    ...
    fn render_git_operations_autogen_section(
        ...
        Flex::column()
            .with_child(render_ai_setting_toggle::<GitOperationsAutogenEnabled>(
                i18n::t("settings.ai.commit_pull_request_generation"),
                AISettingsPageAction::ToggleGitOperationsAutogen,
    ...
    fn render(
        ...
        if FeatureFlag::ProfilesDesignRevamp.is_enabled() {
            ...
            agents_header.add_child(render_ai_setting_description(
                i18n::t("settings.ai.agents_description"),
                ai_settings.is_any_ai_enabled(app),
                app,
            ));
...
impl AgentsWidget {
            ...
            .with_child(
                Container::new(
                    render_ai_setting_description(
                        i18n::t("settings.ai.profiles_description"),
                        is_any_ai_enabled,
                        app,
                    )
                )
    ...
    fn render_permissions_section(
        ...
        let permissions_subheader = Container::new(render_custom_size_header(
            appearance,
            i18n::t("settings.ai.permissions"),
            14.0,
            Some(styles::header_font_color(is_any_ai_enabled, app)),
        ))
        .with_margin_bottom(4.0)
        ...
        let code_diffs = self.render_execution_profile_dropdown(
            i18n::t("settings.ai.apply_code_diffs"),
            Icon::Code2,
        ...
        execute_commands_flex.add_child(self.render_execution_profile_dropdown(
            i18n::t("settings.ai.execute_commands"),
            Icon::Terminal,
            ...
            .has_any_overrides()
        {
            widget_children.push(
                Container::new(render_settings_info_banner(
                    i18n::t("settings.ai.workspace_managed_permissions"),
                    None,
                    appearance,
                ))
                .with_margin_bottom(12.0)
        ...
        let write_to_pty = self.render_execution_profile_dropdown(
            i18n::t("settings.ai.interact_with_running_commands"),
            Icon::Workflow,
    ...
    fn render_command_denylist(
        ...
        render_ai_list(
            i18n::t("settings.ai.command_denylist"),
            i18n::t("settings.ai.command_denylist.description"),
            list,
            view,
            ai_settings,
            app,
    ...
    fn render_command_allowlist(
        ...
        render_ai_list(
            i18n::t("settings.ai.command_allowlist"),
            i18n::t("settings.ai.command_allowlist.description"),
            list,
            view,
            ai_settings,
            app,
    ...
    fn render_directory_allowlist(
        ...
        render_ai_list(
            i18n::t("settings.ai.directory_allowlist"),
            i18n::t("settings.ai.directory_allowlist.description"),
            list,
            view,
            ai_settings,
            app,
    ...
    fn render_base_model_setting(
            ...
            Container::new(
                Flex::row()
                    ...
                    .with_children([
                        ...
                        appearance
                            .ui_builder()
                            .span(i18n::t("settings.ai.show_model_picker_in_prompt").to_string())
                            .with_style(UiComponentStyles {
                                font_color: Some(
        ...
        render_dropdown_item(
            appearance,
            i18n::t("settings.ai.base_model"),
            Some(
                i18n::t("settings.ai.base_model.description"),
            ),
    ...
    fn render_codebase_context_outline_generation_setting(
        ...
        let toggle = render_ai_setting_toggle::<CodebaseContextEnabled>(
            i18n::t("settings.ai.codebase_context"),
            AISettingsPageAction::ToggleCodebaseContext,
        ...
        let codebase_context_description = vec![
            FormattedTextFragment::plain_text(
                i18n::t("settings.ai.codebase_context.description"),
            ),
    ...
    fn render_mcp_permissions_zero_state(
        ...
        let header = Container::new(render_body_item_label_with_icon::<AISettingsPageAction>(
            i18n::t("settings.ai.call_mcp_servers").into(),
            Icon::Dataflow,
            Some(styles::header_font_color(
                ai_settings.is_any_ai_enabled(app),
                app,
        ...
        let subtext = {
            let subtext_fragments = vec![
                FormattedTextFragment::plain_text(
                    i18n::t("settings.ai.mcp_zero_state_description"),
                ),
                FormattedTextFragment::hyperlink_action(
                    i18n::t("settings.ai.add_a_server"),
                    AISettingsPageAction::OpenMCPServerCollection,
                ),
                FormattedTextFragment::plain_text(i18n::t("settings.ai.or")),
                FormattedTextFragment::hyperlink(
                    i18n::t("settings.ai.learn_more_mcps"),
    ...
    fn render_mcp_permissions_with_servers(
        ...
        let permission_setting = self.render_execution_profile_dropdown(
            i18n::t("settings.ai.call_mcp_servers"),
            Icon::Dataflow,
            ...
            || current_mcp_setting == ActionPermission::AgentDecides
        {
            let allowlist = self.render_mcp_list(
                i18n::t("settings.ai.mcp_allowlist"),
                i18n::t("settings.ai.mcp_allowlist.description"),
            ...
            || current_mcp_setting == ActionPermission::AgentDecides
        {
            let denylist = self.render_mcp_list(
                i18n::t("settings.ai.mcp_denylist"),
                i18n::t("settings.ai.mcp_denylist.description"),
...
impl SettingsWidget for AIInputWidget {
    ...
    fn render(
        ...
        let show_input_hint_text = render_ai_setting_toggle::<ShowHintText>(
            i18n::t("settings.ai.show_input_hint_text"),
            AISettingsPageAction::ToggleShowInputHintText,
        ...
        widget_children.push(render_ai_setting_toggle::<IncludeAgentCommandsInHistory>(
            i18n::t("settings.ai.include_agent_commands_in_history"),
            AISettingsPageAction::ToggleIncludeAgentCommandsInHistory,
    ...
    fn render_natural_language_detection_section(
        ...
        if FeatureFlag::AgentView.is_enabled() {
            static AUTODETECTION_DESCRIPTION_FRAGMENTS: LazyLock<Vec<FormattedTextFragment>> =
                LazyLock::new(|| {
                    vec![
                        FormattedTextFragment::plain_text(i18n::t("settings.ai.incorrect_detection")),
                        FormattedTextFragment::hyperlink(
            ...
            > = LazyLock::new(|| {
                vec![
                    FormattedTextFragment::plain_text(
                        i18n::t("settings.ai.nld_description"),
                    ),
                    FormattedTextFragment::plain_text(
                        i18n::t("settings.ai.incorrect_input_detection"),
                    ),
                    FormattedTextFragment::hyperlink(
                        i18n::t("settings.ai.let_us_know"),
                        "https://warpdotdev.typeform.com/to/offrTIpq",
            ...
            section.add_children([
                render_ai_setting_toggle::<AIAutoDetectionEnabled>(
                    i18n::t("settings.ai.natural_language_detection"),
                    AISettingsPageAction::ToggleAIInputAutoDetection,
                    is_nld_enabled,
                    is_toggleable,
                    autodetection_toggle,
        ...
        section
            .with_child(render_ai_setting_label::<AICommandDenylist>(
                i18n::t("settings.ai.natural_language_denylist").to_owned(),
                is_toggleable,
                &view.local_only_icon_tooltip_states,
                app,
            ))
            .with_child(render_ai_setting_description(
                i18n::t("settings.ai.natural_language_denylist.description"),
                is_toggleable,
                app,
            ))
            .with_child(
...
impl SettingsWidget for MCPServersWidget {
    ...
    fn render(
        ...
        let header = build_sub_header(
            appearance,
            i18n::t("settings.ai.mcp_servers"),
            Some(styles::header_font_color(is_any_ai_enabled, app)),
        )
        .with_padding_bottom(HEADER_PADDING)
        .finish();
        ...
        let mcp_description = vec![
            FormattedTextFragment::plain_text(
                i18n::t("settings.ai.mcp_description"),
            ),
            FormattedTextFragment::hyperlink(
                i18n::t("settings.ai.learn_more"),
                "https://docs.warp.dev/agent-platform/capabilities/mcp",
            ...
            Some(
                Flex::column()
                    .with_child(render_ai_setting_toggle::<FileBasedMcpEnabled>(
                        i18n::t("settings.ai.auto_spawn_mcp_servers"),
                        AISettingsPageAction::ToggleFileBasedMcp,
                    ...
                    .with_child({
                        ...
                        > = LazyLock::new(|| {
                            vec![
                                FormattedTextFragment::plain_text(
                                    i18n::t("settings.ai.auto_spawn_mcp.description"),
                                ),
                                FormattedTextFragment::hyperlink(
                                    i18n::t("settings.ai.see_supported_providers"),
...
impl AIFactWidget {
    fn render_rules_toggle(
        ...
        let rules_description = vec![
            FormattedTextFragment::plain_text(
                i18n::t("settings.ai.rules.description"),
            ),
            FormattedTextFragment::hyperlink(
                i18n::t("settings.ai.learn_more"),
    ...
    fn render_rule_suggestions_toggle(
        ...
        let toggle = render_ai_setting_toggle::<RuleSuggestionsEnabled>(
            i18n::t("settings.ai.suggested_rules"),
            AISettingsPageAction::ToggleRuleSuggestions,
        ...
        let description = render_ai_setting_description(
            i18n::t("settings.ai.suggested_rules.description"),
            ai_settings.is_any_ai_enabled(app),
            app,
        );
    ...
    fn render_warp_drive_context_toggle(
        ...
        let toggle = render_ai_setting_toggle::<WarpDriveContextEnabled>(
            i18n::t("settings.ai.warp_drive_as_agent_context"),
            AISettingsPageAction::ToggleWarpDriveContext,
        ...
        let description = render_ai_setting_description(
            i18n::t("settings.ai.warp_drive_context.description"),
            ai_settings.is_any_ai_enabled(app),
            app,
        );
...
impl VoiceWidget {
    fn render_voice_section(
        ...
        let mut column = Flex::column().with_child(render_ai_setting_toggle::<VoiceInputEnabled>(
            i18n::t("settings.ai.voice_input"),
            AISettingsPageAction::ToggleVoiceInput,
        ...
        let voice_input_description_text_fragments = vec![
            FormattedTextFragment::plain_text(
                i18n::t("settings.ai.voice_input.description"),
            ),
        ...
        if ai_settings.is_voice_input_enabled(app) {
            column.add_child(render_dropdown_item(
                appearance,
                i18n::t("settings.ai.voice_input_key"),
                Some(i18n::t("settings.ai.voice_input.press_hold")),
                None,
                LocalOnlyIconState::for_setting(
...
impl SettingsWidget for OtherAIWidget {
    ...
    fn render(
        ...
        if FeatureFlag::AgentView.is_enabled() {
            let mut agent_view_column = Flex::column()
                ...
                .with_child(render_ai_setting_toggle::<ShouldRenderUseAgentToolbarForUserCommands>(
                    i18n::t("settings.ai.show_use_agent_footer"),
                    AISettingsPageAction::ToggleUseAgentToolbar,
                ...
                .with_child(render_ai_setting_description(
                    i18n::t("settings.ai.use_agent_footer.description"),
                    is_toggleable,
                    app,
                ));
        ...
        column.add_child(render_dropdown_item(
            appearance,
            i18n::t("settings.ai.agent_thinking_display"),
            Some(i18n::t("settings.ai.thinking_display.description")),
            None,
            LocalOnlyIconState::for_setting(
        ...
        if FeatureFlag::OpenWarpNewSettingsModes.is_enabled() {
            ...
            column.add_child(render_dropdown_item(
                appearance,
                i18n::t("settings.ai.preferred_conversation_layout"),
                None,
                None,
                LocalOnlyIconState::for_setting(
...
impl SettingsWidget for CLIAgentWidget {
    ...
    fn render(
        ...
        let cli_agent_footer_toggle = render_ai_setting_toggle::<ShouldRenderCLIAgentToolbar>(
            i18n::t("settings.ai.show_coding_agent_toolbar"),
            AISettingsPageAction::ToggleCLIAgentToolbar,
        ...
        let description_fragments = vec![
            FormattedTextFragment::plain_text(
                i18n::t("settings.ai.coding_agent_toolbar.description"),
            ),
        ...
        let mut column = Flex::column()
            .with_child(
                build_sub_header(
                    appearance,
                    i18n::t("settings.ai.third_party_cli_agents"),
                    Some(styles::header_font_color(true, app)),
                )
                .with_padding_bottom(HEADER_PADDING)
                .finish(),
        ...
        if is_footer_enabled {
            ...
            if FeatureFlag::CLIAgentRichInput.is_enabled() {
                ...
                let auto_show_toggle_label = render_body_item_label::<AISettingsPageAction>(
                    i18n::t("settings.ai.auto_toggle_rich_input").into(),
                    Some(styles::header_font_color(true, app)),
                    Some(AdditionalInfo {
                        ...
                        tooltip_override_text: Some(
                            i18n::t("settings.ai.requires_warp_plugin").to_owned(),
                        ),
                    }),
                    LocalOnlyIconState::for_setting(
                ...
                column.add_child(
                    render_ai_setting_toggle::<AutoOpenRichInputOnCLIAgentStart>(
                        i18n::t("settings.ai.auto_open_rich_input"),
                        AISettingsPageAction::ToggleAutoOpenRichInputOnCLIAgentStart,
                ...
                column.add_child(render_ai_setting_toggle::<AutoDismissRichInputAfterSubmit>(
                    i18n::t("settings.ai.auto_dismiss_rich_input"),
                    AISettingsPageAction::ToggleAutoDismissRichInputAfterSubmit,
            ...
            let command_list = {
                ...
                list_column.add_child(
                    appearance
                        .ui_builder()
                        .span(i18n::t("settings.ai.commands_enable_toolbar").to_string())
                        .with_style(UiComponentStyles {
                            font_size: Some(CONTENT_FONT_SIZE),
                            ..Default::default()
                        })
...
impl SettingsWidget for AgentAttributionWidget {
    ...
    fn render(
        ...
        let toggle = if state.is_forced_by_org {
            ui_builder
                ...
                .with_tooltip(TooltipConfig {
                    text: i18n::t("settings.ai.org_enforced_setting").to_string(),
        ...
        let toggle_row = build_toggle_element(
            render_body_item_label::<AISettingsPageAction>(
                i18n::t("settings.ai.enable_agent_attribution").to_string(),
        ...
        Flex::column()
            ...
            .with_child(
                build_sub_header(
                    appearance,
                    i18n::t("settings.ai.agent_attribution"),
                    Some(styles::header_font_color(is_any_ai_enabled, app)),
                )
                .with_padding_bottom(HEADER_PADDING)
                .finish(),
            ...
            .with_child(render_ai_setting_description(
                i18n::t("settings.ai.agent_attribution.description"),
                !state.is_disabled,
                app,
            ))
            .finish()
...
impl SettingsWidget for CloudAgentComputerUseWidget {
    ...
    fn render(
        ...
        let toggle_row = build_toggle_element(
            render_body_item_label::<AISettingsPageAction>(
                i18n::t("settings.ai.computer_use_cloud_agents").to_string(),
        ...
        let mut column = Flex::column()
            ...
            .with_child(
                build_sub_header(
                    appearance,
                    i18n::t("settings.ai.experimental"),
                    Some(styles::header_font_color(is_any_ai_enabled, app)),
                )
                .with_padding_bottom(HEADER_PADDING)
                .finish(),
            ...
            .with_child(toggle_row)
            .with_child(render_ai_setting_description(
                i18n::t("settings.ai.computer_use.description"),
                !is_disabled,
                app,
            ));
        ...
        if FeatureFlag::Orchestration.is_enabled() {
            ...
            column.add_child(render_ai_setting_toggle::<OrchestrationEnabled>(
                i18n::t("settings.ai.orchestration"),
                AISettingsPageAction::ToggleOrchestration,
            ...
            column.add_child(render_ai_setting_description(
                i18n::t("settings.ai.orchestration.description"),
                is_any_ai_enabled,
                app,
            ));
        }
...
impl ApiKeysWidget {
    ...
    fn render_api_keys_section(
        ...
        let mut column = Flex::column()
            ...
            .with_child(
                Container::new(
                    render_ai_setting_description(
                        i18n::t("settings.ai.api_keys.description"),
                        is_enabled,
                        app,
                    ))
        ...
        if !is_byo_enabled {
                ...
                UserWorkspaces::as_ref(app).current_team()
            {
                ...
                if team.billing_metadata.customer_type == CustomerType::Enterprise {
                    vec![
                        FormattedTextFragment::hyperlink(i18n::t("settings.ai.contact_sales"), "mailto:sales@warp.dev"),
                        FormattedTextFragment::plain_text(
                            i18n::t("settings.ai.enable_byo_enterprise"),
                        ),
                    ]
                } else {
                    let current_user_email = auth_state.user_email().unwrap_or_default();
                    ...
                    if has_admin_permissions {
                        vec![
                            FormattedTextFragment::hyperlink(
                                i18n::t("settings.ai.upgrade_build_plan"),
                                upgrade_url,
                            ),
                            FormattedTextFragment::plain_text(i18n::t("settings.ai.use_own_api_keys")),
                        ]
                    } else {
                        vec![FormattedTextFragment::plain_text(
                            i18n::t("settings.ai.ask_admin_upgrade"),
                        )]
                    }
                }
            } else {
                ...
                let upgrade_url = UserWorkspaces::upgrade_link(user_id);
                vec![
                    FormattedTextFragment::hyperlink(i18n::t("settings.ai.upgrade_build_plan"), upgrade_url),
                    FormattedTextFragment::plain_text(i18n::t("settings.ai.use_own_api_keys")),
                ]
            };
    ...
    fn render_can_use_warp_credits_with_byok_toggle(
        ...
        let toggle = render_ai_setting_toggle::<CanUseWarpCreditsWithByok>(
            i18n::t("settings.ai.warp_credit_fallback"),
            AISettingsPageAction::ToggleCanUseWarpCreditsWithByok,
        ...
        let description = render_ai_setting_description(
            i18n::t("settings.ai.warp_credit_fallback.description"),
            ai_settings.is_any_ai_enabled(app),
            app,
        );
...
impl AwsBedrockWidget {
    ...
    fn render_aws_bedrock_section(
        ...
        let toggle_description = if is_admin_enforced {
            i18n::t("settings.ai.aws_bedrock_credentials.admin_description").to_string()
        } else {
            i18n::t("settings.ai.aws_bedrock_credentials.description").to_string()
        };

        let mut column = Flex::column().with_spacing(16.).with_child(
            Flex::column()
                .with_child(render_ai_setting_toggle::<AwsBedrockCredentialsEnabled>(
                    i18n::t("settings.ai.use_aws_bedrock_credentials"),
                    AISettingsPageAction::ToggleAwsBedrockCredentialsEnabled,
        ...
        column.add_child(render_input(
            appearance,
            i18n::t("settings.ai.login_command"),
        ...
        column.add_child(render_input(
            appearance,
            i18n::t("settings.ai.aws_profile"),
        ...
        let toggle = render_ai_setting_toggle::<AwsBedrockAutoLogin>(
            i18n::t("settings.ai.auto_run_login_command"),
            AISettingsPageAction::ToggleAwsBedrockAutoLogin,
            auto_login_enabled,
            is_usage_enabled,
        ...
        let description = render_ai_setting_description(
            i18n::t("settings.ai.auto_login.description"),
            is_usage_enabled,
            app,
        );
        column.add_child(
    ...
    fn render(
        ...
        let column = Flex::column()
            ...
            .with_child(
                build_sub_header(
                    appearance,
                    i18n::t("settings.ai.aws_bedrock"),
                    Some(styles::header_font_color(is_any_ai_enabled, app)),
                )
                .with_padding_bottom(HEADER_PADDING)
                .finish(),
