...
impl MCPServersEditPageView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        let save_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(t!("mcp_ext.save"), PrimaryTheme)
        ...
        let reinstall_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(t!("mcp_ext.edit_variables"), PrimaryTheme).on_click(|ctx| {
                ctx.dispatch_typed_action(MCPServersEditPageViewAction::Reinstall);
            })
        });

        let delete_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(t!("mcp_ext.delete_mcp"), DangerSecondaryTheme)
        ...
        let unshare_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(t!("mcp_ext.remove_from_team"), DangerNakedTheme)
        ...
        let editing_disabled_banner = ctx.add_typed_action_view(|_| {
            Banner::new_without_close(BannerTextContent::plain_text(t!(
                "mcp.editing_restricted_to_admins_and_creator"
            )))
    ...
    fn render_header(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        .with_tooltip(move || {
            ui_builder
                .tool_tip(t!("workspace.log_out").to_string())
                .build()
                .finish()
        })
        .build()
        ...
        if contains_secrets {
            ...
            ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                toast_stack.add_ephemeral_toast(
                    DismissibleToast::error(t!("mcp.server_contains_secrets").to_string()),
                    window_id,
                    ctx,
                );
            });
            return Err(t!("mcp.server_contains_secrets").to_string());
        }
    ...
    fn build_templatable_mcp_server_result_from_json(
        ...
        if parsed_templatable_mcp_servers.is_empty() {
            ...
            ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                toast_stack.add_ephemeral_toast(
                    DismissibleToast::error(t!("mcp.no_server_specified").to_string()),
                    window_id,
                    ctx,
                );
            });

            return Err(t!("mcp.no_server_specified").to_string());
        }
            ...
            ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                toast_stack.add_ephemeral_toast(
                    DismissibleToast::error(
                        t!("mcp.cannot_add_multiple_while_editing").to_string(),
                    ),
                    window_id,
                    ctx,
                );
