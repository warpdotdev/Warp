...
impl LocalDiffStateModel {
    ...
    async fn get_file_diff(
        ...
        if commit.is_none() && parsed_hunks.is_empty() && Self::should_retry_with_staged(status) {
            let staged_args = Self::staged_diff_args(file_path_str);
            log::debug!(
            ...
            if let Ok(staged_output) = run_git_command(repo_path, &staged_args).await {
                // The staged content may itself be binary (e.g. a binary
                // file was staged and the worktree reverted to a text HEAD).
                // Re-apply the binary sentinel check so we don't try to
                // parse a `Binary files ... differ` line as text hunks.
                if staged_output
                    .lines()
                    .any(|line| line.starts_with("Binary files ") && line.contains(" differ"))
                {
    ...
    /// retried against the index (`--cached`).
    ///
    /// - `Untracked` / `New`: not yet in the index in a comparable way.
    /// - `Renamed` / `Copied`: have paired old/new paths and their own diff
    ///   shaping; the simple `--cached -- new_path` form would not reproduce
    ///   that. TODO(#10512): consider a rename-aware staged fallback if the
    ///   reverted-rename case surfaces in user reports.
    /// - `Conflicted`: merge conflicts have their own UX and `git diff
    ///   --cached` against an unmerged entry yields stage-2-vs-HEAD, which
    ///   would mis-render in the Code Review panel.
    ///
    /// Everything else (`Modified`, `Deleted`) is eligible — the index can
    /// still differ from HEAD after a working-tree revert, and that's what
    /// we want to surface.
    ...
    fn staged_diff_args(file_path_str: &str) -> Vec<&str> {
        vec![
            "diff",
            "--no-ext-diff",
            "--patch-with-raw",
