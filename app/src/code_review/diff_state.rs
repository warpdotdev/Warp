...
impl DiffStateModel {
    ...
    async fn diff_state_against_head(repo_path: &Path) -> Result<GitDiffWithBaseContent> {
        ...
        for (file_path, status) in changed_files {
            ...
            // Compute entity-level (semantic) diff when feature is enabled.
            // Guarded by local_fs because it reads the current file from disk.
            // Also guarded by diff-size limits to avoid unbounded CPU/memory
            // on large repo-controlled files.
            #[cfg(feature = "local_fs")]
            let entity_diff = if FeatureFlag::SemanticDiff.is_enabled()
                && !file_diff.is_binary
                && !matches!(status, GitFileStatus::Untracked | GitFileStatus::New)
                && content_at_head.is_some()
                && !matches!(file_diff.size, DiffSize::Large | DiffSize::Unrenderable)
            {
                const MAX_ENTITY_DIFF_FILE_SIZE: usize = 4_375_000; // 4.375 MB
            ...
            #[cfg(not(feature = "local_fs"))]
            let entity_diff: Option<languages::semantic_diff::EntityDiff> = None;
