...
fn deserialize_cli_agent<'de, D>(deserializer: D) -> Result<Option<CLIAgent>, D::Error>
where
    D: Deserializer<'de>,
{
    ...
    let serialized_agent = CLIAgent::from_serialized_name(value);
    if serialized_agent != CLIAgent::Unknown || value.eq_ignore_ascii_case("unknown") {
        return Ok(Some(serialized_agent));
    }

    enum_iterator::all::<CLIAgent>()
        .find(|agent| {
            agent.command_prefix().eq_ignore_ascii_case(value)
                || agent.display_name().eq_ignore_ascii_case(value)
        })
        .map(Some)
        .ok_or_else(|| de::Error::custom(format!("unknown CLI agent `{value}`")))
}
...
pub struct TabConfigPaneNode {
    ...
    #[serde(default, deserialize_with = "deserialize_cli_agent")]
...
fn resolve_pane_node(
    ...
    if let Some(split_direction) = &node.split {
        ...
        let commands: Vec<CommandTemplate> = if let Some(cmds) = node.commands.as_ref() {
            cmds.iter()
            ...
            let command_prefix = agent.command_prefix();
            if command_prefix.is_empty() {
                Vec::new()
            } else {
                vec![CommandTemplate {
                    exec: command_prefix.to_string(),
                }]
            }
        } else {
            Vec::new()
