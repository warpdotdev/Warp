    ...
    V::Action: Clone + From<ContextMenuAction>,
{
    ...
    pub fn show_context_menu(&mut self, source: MenuSource, ctx: &mut ViewContext<V>) {
        ...
        if has_selection && can_edit {
            let item = MenuItemFields::new(t!("common.cut").to_string())
        ...
        if has_selection {
            let item = MenuItemFields::new(t!("common.copy").to_string())
        ...
        if can_edit {
            let item = MenuItemFields::new(t!("common.paste").to_string())
