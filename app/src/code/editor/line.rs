...
impl EditorLineLocation {
    pub fn line_range(&self) -> &Range<LineCount> {
        match self {
            Self::Current { line_range, .. } => line_range,
            Self::Removed { line_range, .. } => line_range,
            Self::Collapsed { line_range } => line_range,
        }
    }

    pub fn line_number(&self) -> Option<LineCount> {
        match self {
            Self::Current { line_number, .. } => Some(*line_number),
            Self::Removed { line_number, .. } => Some(*line_number),
            Self::Collapsed { .. } => None,
        }
    }
    ...
    pub fn is_same_line(&self, other: &Self) -> bool {
        match (self, other) {
            (Self::Current { line_number: a, .. }, Self::Current { line_number: b, .. })
                if a == b =>
            {
                true
            }
    ...
    pub fn into_render_line_location(self) -> RenderLineLocation {
        match self {
            Self::Current { line_number, .. } => RenderLineLocation::Current(line_number),
            Self::Removed {
                line_number, index, ..
