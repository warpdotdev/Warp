...
File and crate renames are the worst case for git merges; doing this last keeps upstream cherry-picks clean for as long as possible. By the time 05 starts, 02 has already removed the AI surface (a chunk of the brand-bearing code), so there's less to rename.
...
- [ ] **5a — Audit doc.** Enumerate every file/identifier/asset, classify each as rename / replace / keep / regenerate. Output: `roadmap/05-rebrand/AUDIT.md`. No code change. Drives 5b–5j.
- [ ] **5b — Workspace + crate renames.** All `warp_*` → `twarp_*`, `warpui*` → `twarpui*`. Updates `Cargo.toml` workspace + every `use` import.
- [ ] **5c — CLI binary + URL scheme.** `app/Cargo.toml` binaries (`warp` → `twarp`, `warp-oss` → `twarp-oss`); `warp://` URI scheme → `twarp://`.
- [ ] **5d — Bundle IDs + native plists.** `dev.warp.*` → `dev.twarp.*` across all 5 channels; rename `WarpDockTilePlugin.{m,h}`; update `DockTilePlugin/Info.plist`, `CLI-Info.plist`.
- [ ] **5e — Brand assets.** Replace logo SVGs, regenerate channel icon sets, rename files (`warp-*` → `twarp-*`), update references.
- [ ] **5f — User-facing strings.** UI labels, error messages, help text, settings, about page, onboarding copy.
- [ ] **5g — Build scripts + installers.** Linux RPM/deb/Arch/AppImage, Windows `.iss`, macOS bundling, GitHub workflow filenames.
- [ ] **5h — Servers + telemetry.** Stub or redirect `app.warp.dev` URLs (auto-update, login, telemetry). After 02 the surface is much smaller.
- [ ] **5i — Internal feature renames.** `warp_drive` → `twarp_drive`, `warpify` → `twarpify`, `warp_pack` → `twarp_pack`, `open_in_warp` → `open_in_twarp`. Also `.warp/` → `.twarp/`, `.warpindexingignore` → `.twarpindexingignore`, `WARP.md` → `TWARP.md`.
- [ ] **5j — Cleanup sweep.** `rg` for any remaining `\b[Ww]arp\b`, manually classify. Update `Cargo.toml` workspace `authors`. Final pass.
...
- Cherry-pick cost spikes after 5b — bias toward batching upstream pulls just before 5b lands.
