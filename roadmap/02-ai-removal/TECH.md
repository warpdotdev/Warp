...
3. **Delete the onboarding agent slide.** Remove `crates/onboarding/src/slides/agent_slide.rs`. Remove its registration in the slide list (search for `agent_slide` / `AgentSlide` in `crates/onboarding/src/`). Update `crates/onboarding/src/model.rs:86-91` so the intent-picker no longer forks on AI; the Terminal intent is the only AI-relevant intent (or, if the intent picker has more than one option for non-AI reasons, leave the intent enum alone and remove only the `disable_oz` mapping).
4. **Delete the settings AI page.** Remove `app/src/settings_view/ai_page.rs` and its registration in the settings sidebar (search for `ai_page` in `app/src/settings_view/mod.rs` or its parent).
