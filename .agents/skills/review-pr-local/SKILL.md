...
- Author environment limitations (e.g., headless runner, no desktop, environment can't capture) do not exempt UI-impacting changes from visual evidence. If such reasons are given, request evidence from a local desktop or supported remote environment and set the verdict to `Request changes`.
