...
- **Tool-call visibility (Slice B)**. Claude is invoked with `--output-format stream-json --verbose`; tool activity is surfaced as inline text annotations (`[tool: Bash] echo hello` / `[result] hello`). The `[tool: Bash]` command annotation is capped at 120 chars (intentional one-liner). Tool result text is capped at 4000 chars; longer output shows `... (truncated, full output N chars)` inline.
...
- **Tabs persist across restarts (Part 1)**. Tab state was already written to `warp.sqlite` via the `workspace:save_app` action and the `WindowSnapshot` serialization path. On relaunch the `NewWorkspaceSource::Restored` branch in `workspace/view.rs` restores all tabs, their pane layouts, and active-tab index. No code change needed; verified by inspection.
- **Per-tab working directory restored on restart (Slice A)**. `TerminalPaneSnapshot.cwd` is already captured via `pwd_if_local()` on each snapshot and written to `warp.sqlite`. On restore, `pane_group/mod.rs` converts it back to a `startup_directory` and passes it to `create_session`. The gap was that `on_will_terminate` (called on `pkill`/Cmd-Q) never flushed an up-to-date snapshot before terminating the writer thread, so any cwd change since the last periodic save was lost. Fix: `on_will_terminate` in `lib.rs` now dispatches `workspace:save_app` before `writer.terminate()`, ensuring the snapshot reaches the channel first and the writer drains it before exiting. Terminal block history is still not restored (no block serialization); that is a separate follow-up.
- **Conversation history persists across restarts (Part 2 - Slice B)**. Under bypass mode, AI panel conversations are written to `warp.sqlite` via `write_updated_conversation_state` (which fires at stream completion). Previously, local-CLI conversations only contained `AgentOutput` messages; `initialize_historical_conversations` requires at least one `UserQuery` message to extract the `initial_query` and silently dropped all bypass-mode conversations on restart. Fix: `local_cli.rs` now emits an `AddMessagesToTask` event with a `UserQuery` message immediately after `CreateTask` for new (non-resumed) conversations. This stores the user query text in the task's protobuf message list in SQLite. On the next launch, the conversation appears in the history sidebar (left panel). Also fixed: `generate_multi_agent_output` in `impl.rs` now routes to the local CLI when `local_model_routing_active()` is true (i.e. `WARP_BYPASS_AUTH=1` set, regardless of `WARP_LOCAL_AI`), instead of only when `is_active()` is true. This ensures the model-selector-driven path (`local:claude:sonnet` etc.) correctly bypasses the server even without an explicit `WARP_LOCAL_AI` env var. The cloud path for authenticated users is untouched.
...
- **AI command suggestions on shell errors**. When a typed shell command fails with a recognised error pattern (`No such file or directory`, `command not found`, `permission denied`, `not a git repository`, `unknown option`, `syntax error`, ~17 patterns total) or exit code 127, Warp spawns the local CLI in the background (`claude -p` or `codex exec`) with a terse prompt asking for a corrected command. The suggestion (capped at 200 chars) appears as ghost-text in the input box - press Tab or right-arrow to accept, type to dismiss. Fires at most once per failed block. Behind `local_ai::auth_bypass_enabled()`.
- **Claude and Codex session reuse across turns**. Each `conversation_id` is mapped to a Claude `session_id` (or Codex `thread_id`) in a process-wide cache. Follow-up turns pass `--resume <session_id>` (Claude) or `codex exec resume <thread_id>` (Codex), skipping CLI startup cost (~300-700ms saved) and avoiding resending conversation context. The session ID is extracted from the first `system` event in the stream-json output (Claude) or `thread.started` event (Codex). The first turn of any conversation always starts fresh. On non-zero exit the cache entry is evicted so the next turn spawns fresh. Behind `local_ai::is_active()`.
