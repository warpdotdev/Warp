...
#### Git LFS
The input-classifier ONNX model binaries under `crates/input_classifier/models/**` are stored via Git LFS. If you clone the repo without `git-lfs` installed, those files will be ~130-byte pointer stubs and the app will fail to load the classifier (silently falling back to the heuristic classifier). Install git-lfs (`brew install git-lfs` on macOS, `sudo apt-get install git-lfs` on Linux, `winget install GitHub.GitLFS` on Windows) and run `git lfs install && git lfs pull` once per checkout. `./script/bootstrap` automates this step. Tokenizer JSON files are explicitly excluded from LFS so they remain diff-able.
